...
/* 1. 기간을 고르면 각 페이지에서 필터됨(포스트가 각 페이지에 배속되지 않게 필요) -> 완료
   2. 새로고침 했을 때 현재 페이지에서 머물러야 함
   3. 기간 설정 시 page번호도 변경이 필요하다. -> 완료
   4. 페이지 번호가 처음 렌더링 할 때 부터 나와야 한다. -> 완료
   5. 2페이지에서 기간 설정해서 포스트 갯수가 줄어들면 1페이지로 넘어와야 하는데 2페이지에서 머문다.
*/
  ...
  return (
      <div style={{ paddingTop: "20px", width: "100%" }}>
        ...
        <div style={{ fontSize: "1.8rem", fontWeight: "800", textAlign: "left"
        , marginLeft: "calc((100% - 5 * 180px - 5 * 30px - 5 * 6px) / 2 + 35px)", marginBottom: "20px" }}>
          {getCountryInKorean()}</div>
        <hr style={{ width: "calc(1.8rem * 25)", marginLeft: "calc((100% - 5 * 180px - 5 * 30px - 5 * 6px) / 2 + 30px)", marginBottom: "20px" }} />
        <div
          className="country-posts"
          style={{ width: "calc(5 * 180px + 5 * 30px + 5 * 6px)", height: "450px" }} // total width 고정 필요
        >
        ...
        <div>
      {pages.map(page => (
          <button key={page} onClick={showCurrentPage} style={{margin: "0 5px 100px 5px", backgroundColor: "white", border:"none", fontSize: "1.4rem"}}>{page}</button>
        ))}
        </div>
      </div>
  );
